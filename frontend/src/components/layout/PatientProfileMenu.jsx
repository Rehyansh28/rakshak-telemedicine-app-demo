...
export default function PatientProfileMenu() {
  ...
  const items = [
    ...
    },
    {
      ...
      onClick: () => {
        close();
        navigate(PATHS.patient.waitingRoom);
      },
    },
    {
      icon: Shield,
    ...
    <div className="relative" ref={ref}>
      ...
      >
        <div className="h-8 w-8 rounded-full bg-primary-container flex items-center justify-center shrink-0">
          <User className="w-4 h-4 text-secondary-container" />
        </div>
        <span className="hidden md:block text-xs font-medium text-primary max-w-[100px] truncate">
          {soldier.rank}
        </span>
        <ChevronDown className={`w-3.5 h-3.5 text-on-surface-variant transition-transform ${profileOpen ? 'rotate-180' : ''}`} />
      </button>
        ...
        {profileOpen && (
          ...
          >
            <div className="p-4 border-b border-outline-variant/30 bg-surface-container-low">
              <p className="font-sora font-semibold text-sm text-primary">{soldier.name}</p>
